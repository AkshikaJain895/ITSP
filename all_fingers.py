...
previous_positions = {i: [] for i in fingertip_indices}
smoothing_factor = 5
...
def smooth_positions(positions, landmark_index):
    if not positions:
        return None
    
    if len(previous_positions[landmark_index]) < smoothing_factor:
        previous_positions[landmark_index].append(positions)
        return positions
    else:
        previous_positions[landmark_index].pop(0)
        previous_positions[landmark_index].append(positions)

    avg_positions = np.mean(previous_positions[landmark_index], axis=0).astype(int)
...
def detect_hands(image):
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            for i in fingertip_indices:  # Iterate through all fingertip landmarks
                landmark = hand_landmarks.landmark[i]
                h, w, _ = image.shape
                cx, cy = int(landmark.x * w), int(landmark.y * h)
                smoothed_positions = smooth_positions((cx, cy), i)
                if smoothed_positions is not None:
                    cv2.circle(image, tuple(smoothed_positions), 5, (0, 255, 0), -1)
